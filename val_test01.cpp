...
//
{
  ...
  // We only allocated space for size n, so the maximum accessible index should be n-1. 
  for ( i = 2; i < n; i++ )
  {
    x[i] = x[i-1] + x[i-2];
    cout << "  " << i << "  " << x[i] << "\n";
  ...
  // Should use free to free space allocatd by malloc. Using delete would cause undefined bahavior. 
  free(x); 
